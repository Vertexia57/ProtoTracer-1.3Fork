...
void MenuHandler<menuCount>::Update() {
    ...
    if (!pinState && inputCount > 0 && timeOn > 500 && inputCount <= 4) {
        uint32_t mask = 0;
        ...
        if (val == 20) {
            ...
            currentMenu = (currentMenu) % (menuCount - 1) + 1;
            ...
            switch (val) {
                ...
                case 9:
                    ...
                    val = currentValue[currentMenu] - 1;
                    if (maxValue[currentMenu] > 0 && val == B11111111) val = maxValue[currentMenu] - 1;
                    currentValue[currentMenu] = val;
                    break;
                case 13:
                    // M (2 long presses) is used to go back a menu
                    WriteEEPROM(currentMenu, currentValue[currentMenu]);
                    currentMenu = (currentMenu + menuCount - 3) % (menuCount - 1) + 1;
                    break;
                case 15:
            ...
            if (maxValue[0] > 0) val = val % maxValue[0]; // If max is set, wrap value
            currentValue[0] = val;
        }
