...
RGBColor SpiralMaterial::GetRGB(const Vector3D& position, const Vector3D& normal, const Vector3D& uvw) {
    ...
    int endBox = ceil(ratio * colorCount);

    RGBColor bottom = rgbColors[startBox];
    RGBColor top = rgbColors[endBox % colorCount];
    RGBColor between = RGBColor::InterpolateColors(bottom, top, ratio * colorCount - startBox);
